const { Instansi, sequelize } = require('./src/models');
...
async function sync() {
  try {
    console.log('Syncing Instansi schema...');
    await Instansi.sync({ alter: true });

    console.log('Upserting 58 instansi entries...');
    let successCount = 0;
    ...
    console.log(`SUCCESS: ${successCount} instansi entries synced successfully.`);
    process.exit(0);
  } catch (err) {
    console.error('ERROR syncing instansi:', err);
    process.exit(1);
  }
}
